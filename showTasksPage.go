...
func showTasksPage(writer http.ResponseWriter, request *http.Request) {
	...
	fmt.Fprintln(writer, `
		...
		<form method="GET">
			...
			<button class="item4" type="submit" name="action" value="Button 1">Task List 1</button>
			<button class="item4" type="submit" name="action" value="Button 2">Task List 2</button>
			<button class="item4" type="submit" name="action" value="Button 3">Task List 3</button>
	...
	if request.Method == http.MethodGet && request.URL.Query().Get("action") != "" {
		...
		if buttonPressed == "the Reset button" { //reset button
			...
			buttonNum := string(buttonPressed[7])
